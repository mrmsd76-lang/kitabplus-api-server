...
startServer().catch(console.error);
  ...
  app.get("/api/debug/session-users", async (_req, res) => {
    try {
      ...
      const OWNER_OPEN_ID = process.env.OWNER_OPEN_ID ?? 'NOT_SET';
      
      const resp = await fetch(`${SUPABASE_URL}/rest/v1/users?select=id,openId,name,email,role,lastSignedIn&order=id.asc`, {
        headers: {
      ...
      return res.json({
        ok: true,
        ownerOpenId: OWNER_OPEN_ID || 'NOT_SET',
        sessionUserCount: users.length,
        sessionUsers: users.map((u: any) => ({
          id: u.id,
          openId: u.openId,
          name: u.name,
          email: u.email,
